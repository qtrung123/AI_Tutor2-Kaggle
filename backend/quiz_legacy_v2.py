"""Legacy Quiz Generation V2 engines, kept for comparison and their regression tests.

Not on the live path: backend.quiz_service.generate_quiz uses the "Study Units" pipeline
(backend/quiz_units.py, quiz_service._generate_quiz_from_units) for both scopes. This module holds
the Planner-based topic engine (_generate_topic_quiz_v2), the document-scope blueprint engines
(_run_document_v2_batch, _run_document_single_choice_quiz) and the older batch generator
(_generate_quiz_batch), plus the helpers only they use.
"""
import difflib
import json
import math
