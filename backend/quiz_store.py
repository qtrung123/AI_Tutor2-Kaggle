    ...
    with _connect() as connection:
        ...
        migrations = {
            "quizzes": {
                ...
                # Set only for quizzes generated by the admin Quiz model benchmark; NULL otherwise.
                "benchmark_id": "TEXT",
            },
            "quiz_questions": {
...
# The admin Quiz model benchmark (backend/model_benchmark_service.py) opens this around each
# generate_quiz() call, so the quiz is marked as benchmark output in the same INSERT that saves it --
# no pipeline parameter, and no window in which it could show up as a learner quiz.
_benchmark_id: ContextVar[str | None] = ContextVar("quiz_store_benchmark_id", default=None)


@contextmanager
def saving_benchmark_quizzes(benchmark_id: str):
    """Mark every quiz saved inside this block as generated by benchmark `benchmark_id`.

    Benchmark quizzes stay loadable by exact quiz_id (get_quiz_by_id) for inspection, but are
    excluded from the Quiz Library, quiz reuse (get_quiz), Study Planner activity and quiz history.
    """
    token = _benchmark_id.set(str(benchmark_id))
    try:
        yield
    finally:
        _benchmark_id.reset(token)
...
def _insert_quiz(connection: sqlite3.Connection, document_id: str, difficulty: str, quiz: dict, owner_id: str = LEGACY_USER_ID) -> dict:
    ...
    stored = {
        **quiz,
        ...
        "benchmark_id": _benchmark_id.get() or quiz.get("benchmark_id") or None,
    }
    ...
    connection.execute(
        ...
        INSERT INTO quizzes (
            ...
            assessment_scope, assessment_plan_json, planner_version, owner_id, benchmark_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            quiz_id,
...
def list_document_quizzes(document_id: str, owner_id: str = LEGACY_USER_ID) -> dict[str, dict]:
    ...
    with _connect() as connection:
        rows = connection.execute(
            ...
            " AND benchmark_id IS NULL ORDER BY created_at DESC",
