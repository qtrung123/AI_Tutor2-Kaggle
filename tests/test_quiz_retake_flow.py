...
class QuizRetakeFlowTests(unittest.TestCase):
    ...
    def test_frontend_uses_deferred_submission_navigation_review_and_separate_regeneration(self):
        ...
        self.assertIn("renderCompletedQuizReview", script)
        self.assertIn('navigator.className = "quiz-review-navigator"', script)
        self.assertIn('button.className = `quiz-review-nav-button ${result?.is_correct ? "correct" : "incorrect"}`', script)
        self.assertIn('number.textContent = `Question ${index + 1}/${total}`', script)
        self.assertIn("quizQuestionIndex += direction", script)
        self.assertIn("${correctCount} correct", script)
        self.assertIn("${total - correctCount} incorrect", script)
        self.assertIn('explain.textContent = quizExplanationPending ? "Explaining..." : "Explain Answer"', script)
        self.assertIn("explainReviewedQuestion(question, result, explain)", script)
        self.assertIn("quizExplanationPending || chatForm.classList.contains", script)
        self.assertIn('tutorLayout.classList.add("quiz-explanation-open")', script)
        self.assertIn("await sendTutorMessage(message)", script)
        self.assertIn("Give a concise explanation grounded in the current document.", script)
        self.assertIn("result.correct_answer", script)
        self.assertIn("result.selected_answer", script)
        review_card = script[script.index("function createAssessmentReviewCard") : script.index("function reviewedAnswerText")]
        self.assertNotIn('className = "quiz-explanation"', review_card)
